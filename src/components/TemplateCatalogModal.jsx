...
export default function TemplateCatalogModal({ isOpen, onClose, selectedTemplate, onSelectTemplate }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-black/85 backdrop-blur-xl animate-fadeIn">
      <div className="bg-slate-900/95 border border-slate-700/80 w-full max-w-6xl rounded-3xl p-5 sm:p-8 shadow-2xl relative text-slate-100 max-h-[92vh] flex flex-col ring-1 ring-white/10">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-5 right-5 p-2 rounded-2xl text-slate-400 hover:text-white bg-slate-800/80 hover:bg-slate-700 transition-colors border border-slate-700/60"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-slate-800">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-xl sm:text-2xl font-black text-white tracking-tight">
                  Universal Design Gallery
                </h3>
                <span className="text-[11px] font-extrabold px-2.5 py-0.5 rounded-full bg-gradient-to-r from-blue-500/20 to-indigo-500/20 text-blue-300 border border-blue-500/40">
                  22 Layouts
                </span>
              </div>
              <p className="text-xs text-slate-400 mt-0.5">
                Engineered for maximum recruiter readability, algorithmic ATS passing, and dual orientation
              </p>
            </div>
          </div>
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full bg-slate-950/80 border border-slate-700/80 rounded-2xl pl-10 pr-3 py-2 text-xs text-slate-100 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
            />
          </div>
        </div>

        {/* Category Pills Filter */}
        <div className="flex items-center gap-1.5 overflow-x-auto py-3.5 border-b border-slate-800/80 scrollbar-none">
          {categories.map((cat) => (
            <button
              key={cat.id}
            ...
            >
              ...
              <span className={`text-[10px] px-1.5 py-0.2 rounded-full font-mono ${
                activeCategory === cat.id ? 'bg-white/20 text-white' : 'bg-slate-800 text-slate-500'
              }`}>
                {cat.count}
              </span>
            </button>
        ...
        <div className="flex-1 overflow-y-auto pt-5 pr-1">
          {filteredTemplates.length === 0 ? (
            <div className="text-center py-16 text-slate-400 text-sm">
              No resume architectures match "{searchQuery}". Try selecting another category tab.
            </div>
          ) : (
                  ...
                  <div
                    ...
                    className={`p-5 rounded-2xl border cursor-pointer transition-all duration-200 flex flex-col justify-between group relative overflow-hidden ${
                      isSelected
                        ...
                        : 'bg-slate-950/50 border-slate-800/90 hover:border-slate-600 hover:bg-slate-850/80 hover:-translate-y-0.5'
                    }`}
                  >
                    <div>
                      {/* Top Bar */}
                      <div className="flex items-center justify-between mb-2.5">
                        <span className="font-extrabold text-sm text-white flex items-center gap-2">
                          <span className="w-2.5 h-2.5 rounded-full shrink-0 shadow-sm" style={{ backgroundColor: tpl.accentColor }}></span>
                          <span className="group-hover:text-blue-300 transition-colors">{tpl.name}</span>
                        </span>

                        <div className="flex items-center gap-1">
                          {tpl.atsScoreGuarantee && (
                            <span className="px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-300 text-[10px] font-bold border border-emerald-500/30 flex items-center gap-1">
                              <ShieldCheck className="w-3 h-3 text-emerald-400" />
                              <span>{tpl.atsScoreGuarantee}</span>
                            </span>
                          )}
                          {isSelected && (
                            <span className="px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-300 text-[10px] font-bold border border-blue-500/40 flex items-center gap-1">
                      ...
                      {/* Tagline */}
                      <p className="text-xs text-blue-300/90 font-medium mb-2 leading-relaxed">
                        {tpl.tagline}
                      </p>

                      {/* Recommended For */}
                      <p className="text-[11.5px] text-slate-400 mb-3 leading-relaxed">
                        <strong className="text-slate-300">Recommended for:</strong> {tpl.recommendedFor}
                      </p>

                      {/* Key Reasons */}
                      {tpl.bestMatchCriteria?.reasons && (
                        <ul className="space-y-1 text-[11px] text-slate-400 border-t border-slate-800/80 pt-2.5 mb-3">
                    ...
                    <div className="pt-3 border-t border-slate-800/60 flex items-center justify-between gap-2 mt-auto">
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] px-2 py-0.5 rounded-md bg-slate-900 text-slate-400 border border-slate-800 font-bold uppercase tracking-wider font-mono">
                          {tpl.category}
                        </span>
                        {tpl.supportsLandscape && (
                          <span className="text-[10px] px-2 py-0.5 rounded-md bg-indigo-950/60 text-indigo-300 border border-indigo-700/40 font-medium flex items-center gap-1">
                      ...
                      >
                        <span>{isSelected ? 'Selected' : 'Switch'}</span>
